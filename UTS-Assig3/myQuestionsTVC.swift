...
class myQuestionsTVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return theseQuestions.count
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        let cell = tableView.dequeueReusableCell(withIdentifier: "QuestionCell", for: indexPath)
        cell.textLabel?.text = theseQuestions[indexPath.row].qTitle
        currentQuestion = theseQuestions[indexPath.row]
        cell.detailTextLabel?.text = "Swift Quizz 3 - Practice question1"
    ...
    */
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "questions2question" {
            let theQuestionsVC = segue.destination as! theQuestionVC
            theQuestionsVC.thisQuestion = currentQuestion
        }
    }
